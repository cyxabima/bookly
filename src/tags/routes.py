...
from src.books.schemas import Book, BookDetailsModel
...
async def add_tag(
    tag_data: TagCreateModel, session: AsyncSession = Depends(get_db_session)
):
...
@tags_router.post(
    "/book/{book_uid}/tags", response_model=BookDetailsModel, dependencies=[user_role_checker]
)
...
async def delete_tag(
    tag_uid: str, session: AsyncSession = Depends(get_db_session)
) -> None:
